...
int copy_image(int dir, unsigned long image_size) 
{
	...
	if (dir == 1) {
		...
		ret = raspi_write((uchar *)CFG_LOAD_ADDR, CFG_KERN2_ADDR, image_size);
#endif
	}
	else if (dir == 2) {
#if defined (CFG_ENV_IS_IN_NAND)
		...
		if (image_size <= len) {
			...
			ret = raspi_write((uchar *)CFG_LOAD_ADDR, CFG_KERN_ADDR, image_size);
		}
		else {
			...
			memcpy(CFG_LOAD_ADDR, (void *)CFG_KERN2_ADDR, image_size);
			ret = raspi_write((uchar *)CFG_LOAD_ADDR, CFG_KERN_ADDR, len);
			ret = raspi_write((uchar *)(CFG_LOAD_ADDR + len), PHYS_FLASH_2, image_size - len);
		}
#else
		e_end = CFG_KERN_ADDR + image_size - 1;
		if (get_addr_boundary(&e_end) != 0)
